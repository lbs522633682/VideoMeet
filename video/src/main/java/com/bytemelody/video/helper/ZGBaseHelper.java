package com.bytemelody.video.helper;

import android.app.Application;
import android.content.Context;

import com.bytemelody.video.utils.AppLogger;
import com.bytemelody.video.utils.PreferenceUtil;
...
public class ZGBaseHelper {
    ...
    public void initZGBase(Context context) {

        String randomSuffix = "-" + new Date().getTime()%(new Date().getTime()/1000);

        String userId = DeviceInfoManager.generateDeviceId(context) + randomSuffix;
        String userName = DeviceInfoManager.getProductName() + randomSuffix;


        // 使用Zego sdk前必须先设置SDKContext。
        ZGBaseHelper.sharedInstance().setSDKContextEx(userId, userName, null, null, 10 * 1024 * 1024, (Application) context);

        PreferenceUtil.getInstance().initPreference(context);


        // TODO bugly初始化用户id
        //CrashReport.initCrashReport(context), "7ace07528f", false);
